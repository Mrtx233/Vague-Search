...
# bing_3.py 在 stdout 打印运行结果时使用的前缀
RUN_RESULT_PREFIX = "RUN_RESULT_JSON:"
# 默认 json 文件夹（可按需修改）
DEFAULT_JSON_DIR = r"E:\Crawler\模糊搜索\模糊搜索\json\output\法语"
...
def run_bing_once(script_path: Path, json_path: Path) -> Tuple[int, Optional[Dict]]:
    """单次运行 bing_3.py，并返回 (return_code, run_result)。"""
...
def main(default_json_dir: str = "") -> int:
    """批量调度入口：逐个 json 运行 bing_3.py。"""
    parser = argparse.ArgumentParser(description="批量调度 bing_3.py 逐个运行 json")
    parser.add_argument(
        "json_dir",
        nargs="?",
    ...
    script_path = Path(__file__).with_name("bing_3.py").resolve()
