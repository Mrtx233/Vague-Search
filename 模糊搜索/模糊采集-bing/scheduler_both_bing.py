...
# 默认 json 文件夹（可按需修改）
DEFAULT_JSON_DIR = r"D:\code_Python\Vague-Search\模糊搜索\json\output\印地语"
...
def run_bing_once(script_path: Path, json_path: Path) -> Tuple[int, Optional[Dict]]:
    """单次运行 bing 脚本，并返回 (return_code, run_result)。"""
...
def process_json_files(script_path: Path, json_files: List[Path], args, thread_name: str = "Thread"):
    """处理 JSON 文件列表（单个线程）"""
    ...
    for index, current_json in enumerate(json_files):
        ...
        while True:
            retries += 1
            return_code, run_result = run_bing_once(script_path, current_json)
...
def main(default_json_dir: str = "") -> int:
    """双线程调度入口：同时运行 bing_2.py（正序）和 bing_3.py（逆序）"""
    parser = argparse.ArgumentParser(description="双线程调度 bing_2.py 和 bing_3.py 同时运行")
    parser.add_argument(
        "json_dir",
        nargs="?",
    ...
    script_path_2 = Path(__file__).with_name("bing_2.py").resolve()
    script_path_3 = Path(__file__).with_name("bing_3.py").resolve()

    if not json_dir.exists() or not json_dir.is_dir():
        print(f"[ERR ] json目录不存在或不是目录: {json_dir}")
        return 1
    if not script_path_2.exists() or not script_path_2.is_file():
        print(f"[ERR ] bing_2.py 脚本不存在: {script_path_2}")
        return 1
    if not script_path_3.exists() or not script_path_3.is_file():
        print(f"[ERR ] bing_3.py 脚本不存在: {script_path_3}")
        return 1
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        # 线程1：bing_2.py 正序处理
        future_2 = executor.submit(process_json_files, script_path_2, json_files, args, "bing_2")

        # 为避免两个脚本的模拟浏览器同时启动，延迟 60 秒再启动 bing_3.py
        print("[INFO] 延迟 60 秒后启动 bing_3.py ……")
        time.sleep(60)

        # 线程2：bing_3.py 逆序处理
        reversed_files = list(reversed(json_files))
        future_3 = executor.submit(process_json_files, script_path_3, reversed_files, args, "bing_3")
