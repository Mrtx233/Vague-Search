...
# google_3.py 在 stdout 打印运行结果时使用的前缀
...
def run_google_once(script_path: Path, json_path: Path) -> Tuple[int, Optional[Dict]]:
    """单次运行 google_3.py，并返回 (return_code, run_result)。"""
...
def main(default_json_dir: str = "") -> int:
    """批量调度入口：逐个 json 运行 google_3.py。"""
    parser = argparse.ArgumentParser(description="批量调度 google_3.py 逐个运行 json")
    parser.add_argument(
        "json_dir",
        nargs="?",
    ...
    parser.add_argument(
        "--google-script",
        default=str(Path(__file__).with_name("google_3.py")),
        help="google脚本路径（默认同目录 google_3.py）",
    )
